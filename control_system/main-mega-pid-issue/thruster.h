...
const int forwardOffset = 10;  // Offset for forward movement.

// Thruster settings
int neutralSignal = 1500;
int forwardThrustPercent = 20;
int forwardThrust = neutralSignal + (500 * forwardThrustPercent / 100);
...
void moveForwardWithHeadingControl() {
  ...
  horizontalLeftThrust = constrain(baseSpeed + forwardOffset + pidOutput, 1000, 2000);
  horizontalRightThrust = constrain(baseSpeed + forwardOffset - pidOutput, 1000, 2000);
...
void doPid() {
  // Check for sudden large change in heading
  if (abs(pidInput - pidSetpoint) > 0.4 * pidSetpoint) {
    horizontalRightThrust = neutralSignal;  // Turn off left thruster
    horizontalLeftThrust = neutralSignal; // Turn off right thruster
  ...
  double timeDifference = (currentTime - previousTime) / 1000.0;
  ...
  lastError = error;
  pidPreviousMillis = currentTime;

  int horizontalLeftThrust = constrain(forwardThrust - pidOutput, neutralSignal - maxCorrection, neutralSignal + forwardThrust);
  int horizontalRightThrust = constrain(forwardThrust + pidOutput, neutralSignal - maxCorrection, neutralSignal + forwardThrust);
}
